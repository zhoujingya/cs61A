...
def accumulate(fuse, start, n, term):
    ...
    def recurse(fuse, n, term):
        ...
        if n == 1:
            return term(1)
        return fuse(term(n), recurse(fuse, n-1, term))
        
    return fuse(start, recurse(fuse, n, term))
...
def digit_distance(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def recurse(n1, end = n %10, sum1 = 0): # last n
        if(n1 == 0):
            return sum1
        sum1 += abs(n1 % 10 - end)   
        end = n1 % 10 
        return recurse(n1 // 10, end, sum1) 
    return recurse(n//10)
